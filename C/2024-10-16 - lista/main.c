...
int main() {
    ...
    int opcao, lista[TAM], fim = -1, elemento, posicao;
        ...
        switch (opcao) {
            case 1:
                if (fim < (TAM - 1)) {
                    ...
                    printf("Digite a posicao (0 a %d) que deseja inserir o numero %d: ", fim + 1, elemento);
                    scanf("%d", &posicao);

                    // Verifica se a posição é válida
                    if (posicao < 0 || posicao > fim + 1)
...
LISTA
Implemente uma lista de números inteiros com um menu para inserir, remover e imprimir a lista.
O usuário poderá escolher a posição que deseja inserir o número e também remover.
Não deixe posições vazias entre os elementos.

Insere e remove de qualquer posição, a menos que seja uma lista ordenada
